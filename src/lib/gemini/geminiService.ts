...
function buildAnalysisModelChain(preferredModel?: string) {
  const orderedModels: string[] = ANALYSIS_MODELS.map((item) => item.value);
...
export const analyzeProduct = async (
  ...
  const creativeInstruction = product.creativeIdeas?.trim()
    ? `User Creative Direction (STRICT SOURCE TEXT, do not rewrite unless explicitly asked): ${product.creativeIdeas.trim()}`
    : 'No specific user creative direction provided. Use expert judgment to create the best converting content.';

  const userInputWritingLock = [
    product.description?.trim() ? `User Selling Points / Product Description (STRICT SOURCE TEXT): ${product.description.trim()}` : '',
    product.creativeIdeas?.trim() ? `User Creative Ideas / Script Direction (STRICT SOURCE TEXT): ${product.creativeIdeas.trim()}` : '',
  ].filter(Boolean).join('\n');
...
- 如果用户填写了产品标题、产品描述、卖点诉求或创意想法，发布标题、发布描述、标签和分镜口播必须优先关联并严格服从这些信息；当用户给出明确措辞时，保留用户措辞，不要改写成另一套表达。
